...
SEC_SEARCH_ACTIVE_COMPS_URL = "https://www.sec.gov/cgi-bin/srch-edgar?text=ASSIGNED-SIC%3D{}%20and%20FORM-TYPE%3D10-Q&start={}&count=80&first=2021&last=2021"
        ...
        if (len(filt) == 1):
            ...
            for i in trs:
                try:
                    comp_name = i.text
                    link = i.find('a')['href']
                    # comp_page = requests.get(SEC_BASE_ADDR + link)
                    # browser.get(SEC_BASE_ADDR + link)
                    # time.sleep(4)
                    # html = browser.page_source

                    # now look for comp CIK
                    # comp_soup = BeautifulSoup(comp_page.text, "lxml")
                    ...
                    if (cik_link != None):
                        # in the seach page should be the ticker
                        # final_page = requests.get(SEC_BASE_ADDR + cik_link, timeout = (3, 3.5))
                        # browser.get(SEC_BASE_ADDR + cik_link)
                        # time.sleep(4)
